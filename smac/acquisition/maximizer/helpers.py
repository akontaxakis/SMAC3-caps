...
class ChallengerList(Iterator):
    ...
    ## WHAT FOLLOWS IS EDITED TO ACHIEVE CAPS INTEGRATION
    def get_predefined_challengers(self) -> list:
        """
        Retrieve the list of predefined challengers.
        """
        if self._challengers is None:
            self._challengers = self._challengers_callback()
        return self._challengers

    def remove_selected_challengers(self, selected_indices_set: set) -> None:
        """
        Remove challengers from the internal list based on their indices.

        Args:
            selected_indices_set (set): Set of indices to be removed.
        """
        if self._challengers is None:
            self._challengers = self._challengers_callback()

        # Filter the list based on the indices to be not removed
        self._challengers = [
            val for idx, val in enumerate(self._challengers)
            if idx in selected_indices_set
        ]

        # Reset the index to start iterating from the beginning of the filtered list
        self._index = 0

    def start_random_selection(self) -> None:
        """
        Enable interleaving random configurations.
        """
        if self._random_design is None and self._stored_random_design is not None:
            self._random_design = self._stored_random_design
            self._stored_random_design = None
        elif self._random_design is None:
            self._random_design = ProbabilityRandomDesign(seed=0, probability=0.08447232371720552)

    def stop_random_selection(self) -> None:
        """
        Disable interleaving random configurations and store the current random design.
        """
        if self._random_design is not None:
            self._stored_random_design = self._random_design
            self._random_design = None
