...
from caps.base import CAPS_middleware
from caps.components.lib import config_to_pipeline
...
class ConfigSelector:
    ...
    def __iter__(self) -> Iterator[Configuration]:
        ...
        # CAPS INTEGRATION FOR THE INITIAL STEP OF SMAC
        # USUALLY RANDOM OR META_LEARNING SELECTION
        if self._scenario.mode == "CAPS":
            ...
            N = int(len(scores))
            K = int(len(scores) / 2) + 1
            print(f"\n *** Using CAPS is selecting {K} out of {N} candidate pipelines using the {self._scenario.sel_algo} algorithm.***\n")
            selected_indices_set, sklearn_pipeline_list = CAPS_middleware(sel_algo=self._scenario.sel_algo,
                                                                          lamda=self._scenario.lamda,
                                                                          selection=K,
                                                                          data_id=self._scenario.data_id,
                                                                          random_state=self._scenario.seed,
                                                                          N=N,
                                                                          timeout=self._scenario.trial_walltime_limit,
                                                                          sklearn_pipeline_list=sklearn_pipeline_list,
                                                                          predecessor_scores=scores)
            print("Selected pipelines:", selected_indices_set)
            self._initial_design_configs = [val for idx, val in enumerate(self._initial_design_configs) if
                                            idx in selected_indices_set]
        # CAPS INTEGRATION WITH INIT END
        ...
        while True:
            ...
            #CHALLENGES ARE GENERATED AND WE CALL CAPS AGAIN FOR SELECTION
            if self._scenario.mode == "CAPS":
                sklearn_pipeline_list = []
                scores = []
                ...
                for config in challengers:
                    ...
                    pp = str(predicted_performance[0][0][0])
                    if pp == "nan":
                        scores.append(1)
                    else:
                        scores.append(1 - predicted_performance[0][0][0])
                    if len(sklearn_pipeline_list) == self._scenario.N:
                        break
                print(
                    f"\n *** Using CAPS is selecting {self._scenario.selection} out of {self._scenario.N} candidate pipelines using the {self._scenario.sel_algo} algorithm.***\n")
                selected_indices_set, sklearn_pipeline_list = CAPS_middleware(sel_algo=self._scenario.sel_algo,
                                                                              lamda=self._scenario.lamda,
                                                                              selection=self._scenario.selection,
                                                                              data_id=self._scenario.data_id,
                                                                              random_state=self._scenario.seed,
                                                                              N=self._scenario.N,
                                                                              timeout=self._scenario.trial_walltime_limit,
                ...
                sklearn_pipeline_list = [val for idx, val in enumerate(sklearn_pipeline_list) if
                                         idx in selected_indices_set]
                challengers.remove_selected_challengers(selected_indices_set)
                self._initial_design_configs = [val for idx, val in enumerate(self._initial_design_configs) if
                                                idx in selected_indices_set]
            challengers.restart_index()
            counter = 0
            failed_counter = 0
            for config in challengers:
                if config not in self._processed_configs:
                    ...
                    if retrain:
                        ...
                        if self._scenario.mode:
                            challengers.start_random_selection()
                        break
                else:
                    failed_counter += 1
